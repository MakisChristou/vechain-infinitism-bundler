...
class BundlerServer {
    ...
    async _preflightCheck() {
        ...
        const err = await contracts_1.EntryPoint__factory.connect(this.config.entryPoint, this.provider).callStatic.simulateValidation(emptyUserOp)
            .catch(e => e);
        ...
        const bal = await this.
            provider.getBalance(this.wallet.address);
        console.log('signer', this.wallet.address, 'balance', ethers_1.utils.formatEther(bal));
        if (bal.eq(0)) {
    ...
    async handleMethod(method, params) {
        ...
        switch (method) {
            ...
            case 'debug_bundler_sendBundleNow':
                ...
                break;
            default:
                throw new utils_3.RpcError(`Method ${method} is not supported`, -32601);
        }
        return result;
    }
}
