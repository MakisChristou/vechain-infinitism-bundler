...
function initServer(config, signer) {
    ...
    const reputationManager = new ReputationManager_1.ReputationManager(ReputationManager_1.BundlerReputationParams, (0, utils_1.parseEther)(config.minStake), config.minUnstakeDelay);
    const mempoolManager = new MempoolManager_1.MempoolManager(reputationManager);
    const validationManager = new ValidationManager_1.ValidationManager(entryPoint, reputationManager, config.unsafe);
