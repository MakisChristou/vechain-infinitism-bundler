...
class UserOpMethodHandler {
    ...
    async _validateParameters(userOp1, entryPointInput, requireSignature = true, requireGasParams = true) {
        (0, utils_3.requireCond)(entryPointInput != null, 'No entryPoint param', -32602);
        ...
        (0, utils_3.requireCond)(userOp1 != null, 'No UserOperation param');
        ...
        fields.forEach(key => {
            (0, utils_3.requireCond)(userOp[key] != null, 'Missing userOp field: ' + key + JSON.stringify(userOp), -32602);
            const value = userOp[key].toString();
            (0, utils_3.requireCond)(value.match(HEX_REGEX) != null, `Invalid hex value for property ${key}:${value} in UserOp`, -32602);
        });
    }
    /**
    ...
    async estimateUserOperationGas(userOp1, entryPointInput) {
        const userOp = Object.assign(Object.assign({}, await (0, utils_1.resolveProperties)(userOp1)), { 
            // default values for missing fields.
            paymasterAndData: '0x', maxFeePerGas: 0, maxPriorityFeePerGas: 0, preVerificationGas: 0, verificationGasLimit: 10e6 });
        ...
        if (errorResult.errorName === 'FailedOp') {
            throw new utils_3.RpcError(errorResult.errorArgs.at(-1), Types_1.ValidationErrors.SimulateValidation);
        }
        ...
        const callGasLimit = await this.provider.estimateGas({
            ...
            throw new utils_3.RpcError(message, Types_1.ExecutionErrors.UserOperationReverted);
        });
        validAfter = ethers_1.BigNumber.from(validAfter);
        validUntil = ethers_1.BigNumber.from(validUntil);
        if (validUntil === ethers_1.BigNumber.from(0)) {
            validUntil = undefined;
        }
        if (validAfter === ethers_1.BigNumber.from(0)) {
            validAfter = undefined;
        }
        const preVerificationGas = (0, sdk_1.calcPreVerificationGas)(userOp);
        const verificationGas = ethers_1.BigNumber.from(preOpGas).toNumber();
        return {
            preVerificationGas,
            verificationGas,
            validAfter,
    ...
    async sendUserOperation(userOp1, entryPointInput) {
        ...
        const userOp = await (0, utils_1.resolveProperties)(userOp1);
        console.log(`UserOperation: Sender=${userOp.sender}  Nonce=${(0, utils_3.tostr)(userOp.nonce)} EntryPoint=${entryPointInput} Paymaster=${(0, moduleUtils_1.getAddr)(userOp.paymasterAndData)}`);
    ...
    async getUserOperationByHash(userOpHash) {
        var _a, _b, _c;
        (0, utils_3.requireCond)(((_a = userOpHash === null || userOpHash === void 0 ? void 0 : userOpHash.toString()) === null || _a === void 0 ? void 0 : _a.match(HEX_REGEX)) != null, 'Missing/invalid userOpHash', -32601);
