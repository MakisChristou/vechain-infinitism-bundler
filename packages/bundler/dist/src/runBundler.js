...
async function runBundler(argv, overrideExit = true) {
    ...
    if (config.conditionalRpc && !await (0, utils_2.supportsRpcMethod)(provider, 'eth_sendRawTransactionConditional')) {
        console.error('FATAL: --conditionalRpc requires a node that support eth_sendRawTransactionConditional');
        process.exit(1);
    }
    if (!config.unsafe && !await (0, utils_2.isGeth)(provider)) {
    ...
    const debugHandler = new DebugMethodHandler_1.DebugMethodHandler(execManager, eventsManager, reputationManager, mempoolManager);
